...
class sim_help : public simulator_t {
    public:
        void on_terminate() override {
            ...
            gl->set_montecarlo_current(gl->trans);
...
int main()
{
    ...
    for (size_t i = 0; i < gl->n; i++) {
        sys->add_process( cs::server_t::create_process< my_msg >(
            ...
            TIMESTEP,
            {{"Customers", [gl, &database](auto pt, auto msg){
                ...
                if (p->database[msg->item*2] - p->database[msg->item*2+1] == 0) 
                    ...
                    if (extract <= gl->a) {
                        ...
                        for (size_t i = 0; i < gl->k; i++) {
                            ...
                            else database[i] -= p->database[i*2+1];
                            p->database[i*2+1] = 0;
    ...
    std::cout << "Z " << gl->get_montecarlo_avg() << std::endl
            << "W " << gl->get_montecarlo_avg() / (gl->n * gl->get_horizon()) << std::endl;
    return 0;
}