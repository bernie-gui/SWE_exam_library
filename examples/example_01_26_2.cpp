...
public:
    void fun() override
    {
        ...
        gl->measure.update( isw::uv::count_collisions( proc_list, gl->D ), get_thread_time() );
    }
...
int main()
{
                                     ...
                                     { "M", [gl]( auto &iss )
                                       {
                                           double temp;
                                           iss >> temp;
                                           gl->set_montecarlo_budget( temp );
                                       } } } );
    reader.parse();
    for ( size_t i = 0; i < gl->N; i++ )
    {
        sys->add_process( uv::vehicle_t::create_process(
                              3, gl->T,
                              [gl]( auto )
                              {
                                  return gl->get_random()->uniform_range( static_cast< double >( -gl->L ),
                                                                          static_cast< double >( gl->L ) );
                              },
                              []( auto ) { return 0; },
                              [gl]( auto p )
                              {
                                  ...
                                  std::function< double( std::shared_ptr< std::vector< double > > ) > f =
                                      [gl, pt]( auto v )
                                  {
                                      ...
                                      for ( auto &p : procs )
                                      {
                                          if ( p->get_id().value() == pt->get_id().value() )
                                              continue;
    ...
    sys->add_process( std::make_shared< process_t >( "collision_detector" )
                          ->add_thread( std::make_shared< coll_det_thread >( gl->R, gl->R ) ),
                      "monitors" );
