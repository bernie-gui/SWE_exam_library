...
class my_opt : public optimizer_t<double> {
    public:
        double obj_fun(std::vector<double> &args) override {
            ...
            monty->run();
            return gl->G * gl->p + gl->get_montecarlo_avg();
        }
...
int main()
{
    ...
    auto reader = lambda_parser_t("examples/example_11_25_5.txt", {
        {"A", [gl](auto& iss) { iss >> gl->A; }},
        {"V", [gl](auto& iss) { iss >> gl->V; }},
        // {"B", [gl](auto& iss) { iss >> gl->B; }},
        {"W", [gl](auto& iss) { iss >> gl->W; }},
        {"P", [gl](auto& iss) { iss >> gl->P; }},
        {"B", [gl](auto& iss) { char temp; 
                                                                double temp2; 
                                                                iss >> temp2; 
                                                                iss >> temp;
                                                                if(temp == 'G') {
                                                                    iss >> gl->G;
                                                                    gl->set_optimizer_budget(static_cast<size_t>(temp2));
                                                                }
                                                                else gl->B = temp2;}},
    ...
    my_opt opty(gl);
    opty.optimize(isw::optimizer_strategy::MINIMIZE, 0, 1);   

    output_writer_t out("examples/example_11_25_5out.txt");
    out << "2025-01-09-Mario-Rossi-1234567" << std::endl 
        << "p " << gl->get_optimizer_optimal_parameters()[0] << std::endl
        << "R " << gl->get_optimizer_result() - (gl->G * gl->get_optimizer_optimal_parameters()[0]) << std::endl
