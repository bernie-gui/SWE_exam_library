 ...
 *
 * Description:
 *	This example demonstrates the usage of the simulation library for a specific scenario on January 9, 2025.
 */
#include <cstddef>
#include <cstdlib>
#include <iostream>
...
public:
    ...
    // _montecarlo_current    // cost accumulated
...
public:
    my_input_parser_t( std::shared_ptr< mdp_global > global ) :
        input_parser_t( global, "examples/example-mdp_1.txt" )
    {
    }
...
public:
    void fun() override
    {
        ...
        std::cout << "montecarlo  cost " << global->montecarlo_current() << " cost average"
                  << global->get_montecarlo_avg() << std::endl;
        for ( size_t j = 0; j < global->matrix.size(); ++j )
        {
            ...
            if ( prob > 0.0 && random_value >= sum && random_value < sum + prob )
            {
                ...
                global->set_montecarlo_current( global->montecarlo_current() + cost );
                return;
            }
...
public:
    ...
    {
        auto global = get_global< mdp_global >();
        return global->current_state == global->matrix.size() - 1;
    };
};
...
int main()
{
    ...
    montecarlino->run();
    std::cout << "avg: " << global->get_montecarlo_avg() << std::endl;
}
