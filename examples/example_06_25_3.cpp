...
class cust_thread_1 : public thread_t {
    public:
        void fun() override {
            ...
            auto p = get_process();
            gl->request = gl->get_random()->uniform_range(0, gl->N);
...
class metronome : public thread_t {
    public:
        void fun() override {}

        metronome(double t) : thread_t(t) {}
};
...
class my_sys : public system_t {
    public:
        void on_end_step() override {
            auto gl = get_global<requests_global>();
            if (get_current_time() <= gl->get_horizon() && fmod(get_current_time(), gl->T) == 0) {
                std::cout << get_current_time() << " " << gl->request << std::endl;
                // gl->request = 0;
            }
        }
...
int main()
{
    ...
    // sys->add_network();
    sys->add_process(std::make_shared<process_t>()->add_thread(std::make_shared<metronome>(gl->T)));
